...
exports.config = {
    ...
    logLevel: 'info',
    // screenshotPath: './test/reports/screenshots/', // Saves a screenshot to a given path if a command fails.
    ...
    // afterStep(uri, feature, stepResult, stepData, context) {
    //     if (stepResult.passed !== true) {
    //         const path = this.screenshotPath;
    //         const featureName = uri
    //           .replace(process.cwd(), '')
    //           .split('/')
    //           .join('_')
    //           .replace('.feature', '');
    ...
    //         }${featureName}-${stepData.step.keyword}-${stepData.step.text}.png`;
    ...
    // },
    afterFeature: function (featureResult) {
        browser.closeWindow();
    },
